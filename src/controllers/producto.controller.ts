...
export const getMenu = async (req: Request, res: Response) => {
    try {
        ...
        if (result.length === 0) {
            ...
            return res.status(404).json({ message: 'No hay productos disponibles en al BD.' });
        }
...
export const createProduct = async (req: Request, res: Response) => {
    try {
        ...
        if (!result) {
            ...
            return res.status(404).json({ message: 'No se pudo registrar el nuevo menu.' });
        }
...
export const delProduct = async (req: Request, res: Response) => {
    try {
        const id = Number(req.params.id);
        const detallePedido_idProducto = await ProductoModel.getAllDetallePedido_idProducto(id);
        if (detallePedido_idProducto.length !== 0) {
            console.log(`Producto esta indexado con registros de otra tabla en la BD.`);
            return res.status(404).json({ 'No se puede eliminar, esta indexado a': detallePedido_idProducto });
        }
